...
#if USE_SERIAL            
            uint8_t counter;		 // Counter for correct data framing UART
#else
...
#if USE_SERIAL
			crc ^= pkt->snd->body.read_word.counter;
#else
...
uint8_t* byteStuffing(const uint8_t* input, size_t length, size_t* stuffedLength) 
{
    // Worst case, every byte needs stuffing, so we double the buffer size
    uint8_t* stuffed = malloc(length * 2);
    if (!stuffed) 
    {
        perror("Memory allocation failed for stuffed payload");
        return NULL;
    }
    ...
    *stuffedLength = j;  // Update the stuffed length
    return stuffed;
}
#endif
...
#if USE_SERIAL
				add_uint8(&ptr, pkt->snd->body.read_word.counter);
#else
...
#if USE_SERIAL
		size_t stuffedSize;
		ptr = byteStuffing(buffer, pkt->snd->sizeOfPkt, &stuffedSize);
		free(buffer);
		pkt->snd->sizeOfPkt = stuffedSize;
		buffer = ptr;
#endif
    return buffer;
}
